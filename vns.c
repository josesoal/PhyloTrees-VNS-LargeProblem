...
int VNS(TreePtr phyloTreePtr, ParametersPtr paramsPtr, MultipleLeafPtr *multiple )
{
	...
	/*VNS*/
	score = labelOptimizeTree( phyloTreePtr, paramsPtr, multiple, NULL, 0 );//--method from iterate_tree.c
	while ( iteration <= maxIterations ) {
		...
		while ( k <= kmax ) {
			...
			/* Move or not */
			newScore = labelOptimizeTree( &temporalTree, paramsPtr, multiple, NULL, 0 );//--method from iterate_tree.c
	...
	TreePtr phyloTreePtr, ParametersPtr paramsPtr, int pScore, MultipleLeafPtr *multiple )
{
	...
	while ( TRUE ) {
		...
		/* calculate newScore */ 
		newScore = labelOptimizeTree( &temporalTree, paramsPtr, multiple, NULL, 0 );//from iterate_tree.c
	...
	TreePtr phyloTreePtr, ParametersPtr paramsPtr, int pScore, MultipleLeafPtr *multiple )
{
	...
	while (TRUE){
		...
		if (subtreeRootPtr->leftDescPtr->type == INTERNAL_NODE){
			...
			score1 = labelOptimizeTree( &temporalTree1, paramsPtr, multiple, NULL, 0 );//from iterate_tree.c
			...
			score2 = labelOptimizeTree( &temporalTree2, paramsPtr, multiple, NULL, 0 );//from iterate_tree.c
		}
		...
		if (subtreeRootPtr->rightDescPtr->type == INTERNAL_NODE){
			...
			score3 = labelOptimizeTree( &temporalTree3, paramsPtr, multiple, NULL, 0 );//from iterate_tree.c
			...
			score4 = labelOptimizeTree( &temporalTree4, paramsPtr, multiple, NULL, 0 );//from iterate_tree.c
		}
