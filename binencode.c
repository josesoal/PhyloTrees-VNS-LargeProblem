...
printGenomes ( struct genome_struct *g, int num_genomes, int num_genes )
{
    ...
    fprintf ( stdout, "Genomes:\n\n" );
    for ( i = 0; i < num_genomes; i++ )
    {
        fprintf ( stdout, "(%3d) %16s:", g[i].genome_num, g[i].gnamePtr );
        for ( j = 0; j < num_genes; j++ )
        {
            fprintf ( stdout, "%3d ", g[i].genes[j] );
        }
        fprintf ( stdout, "\n" );
    }
    fprintf ( stdout, "\n" );
...
printGeneAdjMatrix ( int **adj, int num_genes )
{
    ...
    for ( j = 1; j <= num; j++ )
    {
        fprintf ( stdout, "%3d \t", mapToPolarity ( j, num_genes ) );
    }
    fprintf ( stdout, "\n" );

    for ( i = 1; i <= num; i++ )
    {
        fprintf ( stdout, "%3d: \t", mapToPolarity ( i, num_genes ) );
        for ( j = 1; j <= num; j++ )
        {
            fprintf ( stdout, "%3d \t",
...
#if DEBUG
    fprintf ( stdout, "VECTOR GENES:\n" );
    fprintf ( stdout, "width: %d    v: %d\n", width, v );
    for ( i = 0; i < v; i++ )
        fprintf ( stdout, "V[%3d]: ( %4d, %4d)\n", i, geneVectorG0[i],
                  geneVectorG1[i] );
    fprintf ( stdout, "\n" );
#endif
...
{
    int i;
    fprintf ( stdout, "GENOME ENCODINGS:\n" );
    for ( i = 0; i < num_genomes; i++ )
    {
                                      ...
                                      int num_genomes, int num_genes )
{
    ...
    fprintf ( stdout, "HAMMING DISTANCE From ENCODING:\n" );
    for ( i = 0; i < num_genomes; i++ )
    {
                                     ...
                                     int CIRCULAR )
{
    ...
    fprintf ( stdout, "HAMMING DISTANCE From SCRATCH:\n" );
    for ( i = 0; i < num_genomes; i++ )
    {
        for ( j = i + 1; j < num_genomes; j++ )
        {
